...
namespace Clipper2Lib
{
	...
	struct Joiner
	{
		...
		explicit Joiner(OutPt *op1_, OutPt *op2_, Joiner *nexth) : op1(op1_), op2(op2_), nextH(nexth)
		{
			idx = -1;
			next1 = op1->joiner;
			op1->joiner = this;
	...
	Point64 GetIntersectPoint(const Active &e1, const Active &e2)
	{
		...
		else
		{
			...
			q = (b2 - b1) / q;
			return (abs(e1.dx) < abs(e2.dx)) ? Point64(static_cast<int64_t>(e1.dx * q + b1),
	...
	void ClipperBase::CleanCollinear(OutRec *outrec)
	{
		...
		if (!outrec || outrec->is_open ||
			outrec->front_edge || !ValidateClosedPathEx(outrec->pts))
			return;
